...
(function (global, factory) {
  ...
  confug = function(env, passedOptions) {
    ...
    for (iter in _defaultOptions) {
      opts[iter] = _defaultOptions[iter];
    }

    // import options
    if (typeof passedOptions === 'object') {
      for (iter in passedOptions) {
        if (false === _defaultOptions.hasOwnProperty(iter)) {
          throw new Error('Confug Error - Invalid confug option "' + iter + '"');
        }

        opts[iter] = passedOptions[iter];
      }
    }
  ...
  var initStages = confug.initStages = function(stageNames) {
    ...
    for (var i=0; i < stageNames.length; i++) {
      ...
      Config.prototype[stage.abbrev] = stageShorthand(stage);
    }
